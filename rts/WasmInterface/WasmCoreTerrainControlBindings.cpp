	...
	wasmtime_val_raw_t* slots, std::size_t slotCount)
{
	...
	if (state == nullptr || state->native == nullptr || state->native->syncedCtrl == nullptr ||
		state->native->syncedCtrl->terrain == nullptr ||
		state->native->syncedCtrl->terrain->SetHeightMapFunc == nullptr)
		return Trap("SetHeightMapFunc Core binding is unavailable");
	if (slots == nullptr || slotCount != 2)
		return Trap("SetHeightMapFunc Core ABI signature mismatch");
	...
	SetHeightMapFuncQuery query{InvokeCallback, &callback};
	SetHeightMapFuncResult result{};
	state->native->syncedCtrl->terrain->SetHeightMapFunc(&query, &result);
	if (!callback.success)
	...
	std::string& error)
{
	...
	return Define(linker, "set-height-map-func",
