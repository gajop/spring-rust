    pub mod unit_control {
        ...
        pub fn bugger_off(pos: Float3, radius: f32, team_id: i32, options: BuggerOffOptions, exclude_unit_def_i_ds: &[i32]) -> Result<bool> {
            ...
            let __blob1 = { let mut __b = Vec::new(); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if options.spherical { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if options.forced { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&options.exclude_unit_id.to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b };
        ...
        pub fn create_unit(unit_def: &DefRef, pos: Float3, facing: i32, team_id: i32, options: CreateUnitOptions) -> Result<i32> {
            ...
            let __blob2 = { let mut __b = Vec::new(); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if options.build { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if options.flatten_ground { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&options.unit_id.to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&options.builder_id.to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b };
        ...
        pub fn set_unit_build_params(unit_id: i32, param_name: &str, value: NumberOrBool) -> Result<bool> {
            ...
            let __blob1 = { let mut __b = Vec::new(); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&value.number.to_bits().to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if value.boolean { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b.extend_from_slice(&(if value.use_boolean { 1u32 } else { 0u32 }).to_le_bytes()); while !__b.len().is_multiple_of(4) { __b.push(0); } __b };
        ...
        pub fn set_unit_piece_matrix(unit_id: i32, piece_index: i32, matrix: &[f32]) -> Result<bool> {
            let value = crate::generated::unit_control::set_unit_piece_matrix(unit_id, piece_index, { let __values = &matrix[..]; if __values.len() > 16 { return Err(crate::ApiError::new(crate::ErrorCode::InvalidArgument as i32)); } let mut __array = [0 as f32; 16]; __array[..__values.len()].copy_from_slice(__values); __array })?;
            Ok(value)
        }
