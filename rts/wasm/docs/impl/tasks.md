...
### 1a. Results must be generated data, not prose

`benchmarking_results.md` already carries the
`<!-- generated by test/native_api_parity/run_benchmarks_core.py -->` header.
It is the results file. Make it the **only** results file — and **move it to
`rts/wasm/docs/generated/benchmarking_results.md`**. Generated output belongs
in `docs/generated/`; `docs/impl/` is for hand-written documents only. Nothing
generated should ever land in `impl/` again.

Requirements:

- the runner writes it **automatically**, every run, to that path — never to
  `/tmp`. The current default sends the real table to `/tmp/core-bench-bounded.md`
  and it is discarded, which is why drawworld has been reported as 5311, 4434,
  4711, 5150 and 6166 ns across five write-ups with nothing to compare.
- **data only.** No prose, no analysis, no commentary, no "notes" column, no
  conclusions. Tables and a generated header line. Nothing a model typed by hand.
- keep the historical `Wasm (C API, dynamic, CM)` and `Wasm (Rust, typed, CM)`
  columns and `frozen_benchmarks/*/wasm.csv` /
  `frozen_benchmarks/*/wasm_rust_typed.csv` as reference data. They are not
  active comparison backends and must not be used to make current Core claims.
- emit **raw samples**, not only medians, into the per-profile CSVs under
  `test/native_api_parity/frozen_benchmarks/`. With spreads of 1666–4487 ns
  against medians of 1438–5600 ns, a median alone cannot support any claim.
  Include sample count, median, p99 and spread per row.
- re-freeze the baselines. `metadata.json` still says `2026-08-20`, before all
  of this work.
- add a regression check: compare a run against the frozen set and **exit
  non-zero** when a row regresses beyond its noise band. "Core got slower" must
  become a failing command, not a paragraph someone reads.

Analysis, if any is wanted, goes in `impl/core_benchmark_results.md` and stays
out of the generated file. Prefer no analysis at all.

### 1b. Measure the whole surface
...
**Every callin — all 179.** Three of the current eight are synthetic
(`empty`, `unimplemented`, `4modules`) and measure dispatch, not work.

**Callouts across all ten transport classes**, weighted by population:
806 `fixed`, 191 `variable-input-borrowed`, 90 `variable-output-caller-owned`,
89 `dynamic-output-caller-owned`, 71 `handwritten-reviewed`, 52
`variable-io-borrowed-input-caller-owned-output`, 22
`variable-input-nested-adapted`, 16 `variable-input-borrowed-mixed-fixed`,
13 `fixed-option`, 4 `variable-input-adapted`.

The eight transport representatives added last round are **Core-only, with no
Lua peer**, so they answer nothing about whether Core wins. Every row needs a
Lua column.
