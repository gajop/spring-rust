...
fn descriptor_inputs_supported(
    ...
    render_core_wasm_wire::input_descriptor_layout(inputs, records).is_some()
        && inputs
            .iter()
            .all(|field| render_core_wasm_wire::input_field_supported(field, records))
}
...
fn render_callback(
    ...
    for field in &function.inputs {
        if render_core_wasm_wire::direct_type(&field.ty) {
            ...
            direct_slot += 1;
        } else if render_core_wasm_wire::variable_type(&field.ty) {
            query_assign.push_str(&render_core_wasm_wire::render_input_variable(
                field, records,
            ));
        } else {
            query_assign.push_str(&render_core_wasm_wire::render_wire_read(
                &field.ty,
    ...
    if variable_input(plan) {
        let (input_bytes, input_alignment) =
            render_core_wasm_wire::input_descriptor_layout(&function.inputs, records)
