...
pub fn render_cpp(model: &ApiModel) -> String {
    ...
    for module in &model.modules {
        for function in &module.functions {
            ...
            callbacks.push('\n');
            // Only register when the owned SDK actually routes this
            // function to borrowed (Packed32 result + supported inputs).
            // Eligible-but-not-routed functions (Status/FixedOutputBuffer
            // results) are handled by another wire format at runtime.
...
fn render_rust_wrapper(
    ...
    let arity_lint = too_many_arguments_attribute(params.len(), indent);
    let (descriptor_bytes, descriptor_alignment) =
        ...
        .filter(|field| direct_type(&field.ty))
    {
        if let Some(owner) = count_owner(&function.inputs, &field.name) {
            ...
            raw_args.push(rust_direct_arg(
                &field.ty,
                &rust_ident(&field.name.to_snake_case()),
                plan.direct_params[raw_args.len()],
            ));
        }
    }
...
fn rust_direct_arg(ty: &SemanticType, name: &str, core_type: CoreType) -> String {
    let core_name = rust_core_type(core_type);
    match ty {
        SemanticType::Scalar { name: scalar } if scalar == "bool" => {
            if core_type == CoreType::I32 {
                format!("if {name} {{ 1 }} else {{ 0 }}")
            } else {
                format!("(if {name} {{ 1 }} else {{ 0 }}) as {core_name}")
            }
        }
        SemanticType::Enum { .. } => {
            if core_type == CoreType::I32 {
                ...
                format!("{name} as {core_name}")
            }
        }
        SemanticType::Handle { .. } => {
            if core_type == CoreType::I64 {
                format!("{name} as i64")
            } else {
                format!("{name} as {core_name}")
            }
...
fn rust_unpack_packed(ty: &SemanticType, packed: &str) -> String {
    match ty {
        ...
        SemanticType::Scalar { name } => {
            let rust = rust_scalar_type(name);
            if rust == "u32" {
                format!("Ok({packed} as u32)")
            } else {
                format!("Ok(({packed} as u32) as {rust})")
            }
        }
