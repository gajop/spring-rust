    ...
    match &field.ty {
        ...
        SemanticType::Bytes => {
            ...
            output.push_str(&format!(
                "    std::span<const std::uint8_t> {stem}InputWire;\n    if (!state->memory.View({stem}InputPointer, {stem}InputCount, {stem}InputWire)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::OutOfBounds); return nullptr; }}\n    WireReader {stem}InputReader({stem}InputWire);\n    std::uint32_t {stem}InputLength = 0;\n    if (!{stem}InputReader.U32({stem}InputLength)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    std::span<const std::uint8_t> {stem}InputBytes;\n    if (!{stem}InputReader.Bytes({stem}InputLength, {stem}InputBytes) || !{stem}InputReader.Finish(1u)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    query.{field_name} = {stem}InputLength == 0 ? nullptr : reinterpret_cast<std::remove_reference_t<decltype(query.{field_name})>>({stem}InputBytes.data());\n    if (!AssignCoreCount({stem}InputLength, query.{count})) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n",
                field_name = field.name,
            ));
        }
        SemanticType::List { element } if matches!(element.as_ref(), SemanticType::String) => {
            ...
            output.push_str(&format!(
                "    std::span<const std::uint8_t> {stem}InputWire;\n    if (!state->memory.View({stem}InputPointer, {stem}InputCount, {stem}InputWire)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::OutOfBounds); return nullptr; }}\n    WireReader {stem}InputReader({stem}InputWire);\n    std::uint32_t {stem}InputLength = 0;\n    if (!{stem}InputReader.U32({stem}InputLength)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    std::vector<std::string> {stem}InputStrings;\n    std::vector<const char*> {stem}InputPointers;\n    {stem}InputStrings.reserve({stem}InputLength);\n    for (std::uint32_t coreIndex = 0; coreIndex < {stem}InputLength; ++coreIndex) {{\n        std::uint32_t itemLength = 0;\n        if (!{stem}InputReader.U32(itemLength)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n        std::span<const std::uint8_t> itemBytes;\n        if (!{stem}InputReader.Bytes(itemLength, itemBytes)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n        {stem}InputStrings.emplace_back(reinterpret_cast<const char*>(itemBytes.data()), itemBytes.size());\n    }}\n    if (!{stem}InputReader.Finish(1u)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    {stem}InputPointers.reserve({stem}InputStrings.size());\n    for (const auto& item : {stem}InputStrings) {stem}InputPointers.push_back(item.c_str());\n    query.{field_name} = {stem}InputLength == 0 ? nullptr : {stem}InputPointers.data();\n    if (!AssignCoreCount({stem}InputLength, query.{count})) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n",
                field_name = field.name,
            ));
        }
        SemanticType::List { element } if borrowed_list_element(element) => {
            ...
            let (_, element_alignment) =
                fixed_wire_layout(element, records).expect("eligible list input element");
            let cpp = native_cpp_type(element);
            let item_read =
                render_wire_read(element, "item", records, &format!("{stem}InputReader"), 2);
            output.push_str(&format!(
                "    std::span<const std::uint8_t> {stem}InputWire;\n    if (!state->memory.View({stem}InputPointer, {stem}InputCount, {stem}InputWire)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::OutOfBounds); return nullptr; }}\n    WireReader {stem}InputReader({stem}InputWire);\n    std::uint32_t {stem}InputLength = 0;\n    if (!{stem}InputReader.U32({stem}InputLength)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    std::vector<{cpp}> {stem}InputStorage;\n    {stem}InputStorage.reserve({stem}InputLength);\n    for (std::uint32_t coreIndex = 0; coreIndex < {stem}InputLength; ++coreIndex) {{\n        {cpp} item{{}};\n{item_read}        {stem}InputStorage.push_back(item);\n    }}\n    if (!{stem}InputReader.Finish({element_alignment}u)) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n    query.{field_name} = {stem}InputLength == 0 ? nullptr : {stem}InputStorage.data();\n    if (!AssignCoreCount({stem}InputLength, query.{count})) {{ slots[0].i32 = static_cast<std::int32_t>(Status::InvalidArgument); return nullptr; }}\n",
                field_name = field.name,
                item_read = item_read,
            ));
        }
        SemanticType::List { element } => {
            ...
            let (_, element_alignment) = fixed_wire_layout(element, records).expect("eligible list input element");
            let cpp = native_cpp_type(element);
