...
Total Functions: 1280

Callouts (Plugin → Engine): 1093
...
- `RmlUi.LoadFontFace` (params: file_path:string, fallback:boolean?, weight:RmlUi.font_weight?)
...
- `Spring.GetFeaturePieceMatrix` (params: featureID:integer, pieceNum:integer)
...
- `Spring.GetUnitPieceMatrix` (params: unitID:integer, pieceNum:integer)
...
- `Spring.TraceRayGroundInDirection` (params: posX:number, posY:number, posZ:number, dirX:number, dirY:number, dirZ:number, testWater:number?)
