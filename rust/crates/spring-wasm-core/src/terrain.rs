...
pub fn get_water_plane_level() -> Result<f32> {
    #[cfg(target_arch = "wasm32")]
    {
        return super::unpack_f32(unsafe { raw::get_water_plane_level() });
    }
    #[cfg(not(target_arch = "wasm32"))]
    {
        unsupported()
...
pub fn get_ground_blocked(x1: f32, z1: f32, x2: f32, z2: f32) -> Result<bool> {
    #[cfg(target_arch = "wasm32")]
    {
        return super::unpack_bool(unsafe { raw::get_ground_blocked(x1, z1, x2, z2) });
    }
    #[cfg(not(target_arch = "wasm32"))]
    {
        let _ = (x1, z1, x2, z2);
...
pub fn get_ground_normal(x: f32, z: f32, smoothed: bool) -> Result<GroundNormal> {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        let status = unsafe {
            raw::get_ground_normal(x, z, smoothed as i32, pointer as u32 as i32)
        };
        if status != 0 {
            return Err(ApiError::new(status));
        }
        return Ok(GroundNormal {
