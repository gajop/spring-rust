...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--probe-source",
        type=Path,
        help="scan one already-generated probe source instead of generating all contexts",
    )
...
def probe_dependencies(args: argparse.Namespace) -> dict[str, tuple[Callout, ...]]:
    """Recover dependencies for every manifest context.

    The checked-in probe source is intentionally just the last context built by
    the harness.  Scanning it against all manifests therefore made every
    unsynced and UI test look unmapped.  Generate the other context sources in
    a temporary directory so the plan remains a read-only inventory operation.
    """
    if args.probe_source is not None:
        return extract_probe_dependencies(
            args.probe_source.read_text(encoding="utf-8")
        )

    contexts = sorted(
        {
            str(load_json(path).get("context", path.stem.removeprefix("probe_manifest_")))
            for path in manifest_paths(args.parity_root)
        }
    )
    if not contexts:
        return extract_probe_dependencies(PROBE_SOURCE.read_text(encoding="utf-8"))

    dependencies: dict[str, tuple[Callout, ...]] = {}
    with tempfile.TemporaryDirectory(prefix="core-parity-probes-") as temporary:
        temporary_root = Path(temporary)
        for context in contexts:
            output_root = temporary_root / context
            subprocess.run(
                [
                    sys.executable,
                    str(PARITY_ROOT / "generate_probe.py"),
                    "--context",
                    context,
                    "--output-root",
                    str(output_root),
                    "--lua-output",
                    str(output_root / "probe.lua"),
                    "--manifest-output",
            ...
            source = output_root / "src" / "probe_generated.rs"
            dependencies.update(
                extract_probe_dependencies(source.read_text(encoding="utf-8"))
            )
    return dependencies
...
def manifest_paths(root: Path) -> list[Path]:
    # Core manifests are transport-specific execution selections, not the
    # existing Component oracle inventory.  The synced Component manifest has
    # the historical unsuffixed name; a stale `probe_manifest_synced_gadget`
    # file can be left behind by older harness runs and must not become a
    # second oracle for the same context.
    return sorted(
        path
        for path in root.glob("probe_manifest*.json")
        if not path.name.endswith(".core.json")
        and path.name != "probe_manifest_synced_gadget.json"
    )
...
def main() -> int:
    args = parse_args()
    dependencies = probe_dependencies(args)
    ...
    report = {
        ...
        "source": {
            "probe_source": (
                str(args.probe_source.relative_to(ROOT))
                if args.probe_source is not None
                else "temporary context-specific probe sources generated by generate_probe.py"
            ),
