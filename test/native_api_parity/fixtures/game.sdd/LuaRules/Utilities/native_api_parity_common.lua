...
local function generatedParamValue(param, ids)
	...
	if param.generator == "front_vector" then
		return { x = 0, y = 0, z = 1 }
	end
	if param.generator == "right_vector" then
		return { x = 1, y = 0, z = 0 }
	end
...
local function canRunPortableTest(test, ids)
	...
	for _, requirement in ipairs(test.requires or {}) do
		...
		elseif requirement == "ground_point" then
			-- The point is generated by the row itself; it does not require a
			-- discovered fixture object.
		else
			return false
		end
	end
