...
def _report_tests(summary: dict) -> list[str]:
    tests = [str(test) for test in summary["tests"]]
    seen = set(tests)
    for rows in summary.get("rows", {}).values():
        for row in rows:
            test = str(row.get("test", ""))
            if test and test != "complete" and test not in seen:
                tests.append(test)
                seen.add(test)
    return tests


def _resolved_summaries(summaries: list[dict]) -> list[dict]:
    return [
        frozen_benchmarks.fill_missing(summary, REPORT_BACKENDS, FROZEN_ROOT)[0]
        for summary in summaries
    ]


def _indexed(summary: dict) -> dict[str, dict[str, dict]]:
    return {
        backend: {
            str(row["test"]): row
            for row in rows
...
def _timed_metric(row: dict, test: str) -> tuple[float, str] | None:
    if row.get("status") == "unavailable":
        return None
    if "medianNs" not in row and "medianMs" not in row:
        return None
    return base.numeric_metric(row, test)


def _comparison_rows(summary: dict) -> list[tuple[str, dict, dict, dict, tuple[float, str], tuple[float, str], tuple[float, str]]]:
    indexed = _indexed(summary)
    rows = []
    for test in _report_tests(summary):
        if test.startswith("core_ceiling_") or test.startswith("callin_drawworld_"):
            continue
        lua = indexed["lua"].get(test, {})
        native = indexed["native"].get(test, {})
        core = indexed["wasm_core"].get(test, {})
        lua_metric = _timed_metric(lua, test)
        native_metric = _timed_metric(native, test)
        core_metric = _timed_metric(core, test)
        if not (lua_metric and native_metric and core_metric):
            continue
        if not (lua_metric[1] == native_metric[1] == core_metric[1]):
            continue
        if base.ratio_for_test(test, core, lua) == "—":
            continue
        rows.append((test, lua, native, core, lua_metric, native_metric, core_metric))
    return rows


def _ratio_marker(core_metric: tuple[float, str], lua_metric: tuple[float, str]) -> str:
    ratio = core_metric[0] / lua_metric[0]
    marker = "✓" if core_metric[0] < lua_metric[0] else "⚠"
...
def _render_comparison(summaries: list[dict]) -> str:
    ...
    for summary in summaries:
        ...
        for test, _lua, _native, _core, lua_metric, native_metric, core_metric in _comparison_rows(summary):
            lines.append(
                f"| `{profile}` | `{test}` | {base.format_metric(lua_metric)} | "
                f"{base.format_metric(native_metric)} | {base.format_metric(core_metric)} | "
                f"{_ratio_marker(core_metric, lua_metric)} |"
            )
    lines.append("")
    return "\n".join(lines)


def _render_losses(summaries: list[dict]) -> str:
    lines = [
        "<!-- This file is generated by test/native_api_parity/run_benchmarks_core.py. -->",
        "",
    ...
    for summary in summaries:
        profile = str(summary["profile"])
        for test, _lua, _native, _core, lua_metric, _native_metric, core_metric in _comparison_rows(summary):
            if core_metric[0] < lua_metric[0]:
                continue
            lines.append(
                f"| `{profile}` | `{test}` | {base.format_metric(lua_metric)} | "
                f"{base.format_metric(core_metric)} | {_ratio_marker(core_metric, lua_metric)} |"
            )
    lines.append("")
    return "\n".join(lines)


def _render_diagnostics(summaries: list[dict]) -> str:
    lines = [
        "<!-- This file is generated by test/native_api_parity/run_benchmarks_core.py. -->",
        "",
    ...
    for summary in summaries:
        ...
        indexed = _indexed(summary)
        for test in _report_tests(summary):
            if not (test.startswith("core_ceiling_") or test.startswith("callin_drawworld_")):
                continue
            for backend in REPORT_BACKENDS:
                row = indexed[backend].get(test)
                metric = _timed_metric(row or {}, test)
                if metric is not None:
                    lines.append(
                        f"| `{profile}` | `{test}` | `{backend}` | {base.format_metric(metric)} |"
                    )
    lines.append("")
    return "\n".join(lines)


def _render_unpaired(summaries: list[dict]) -> str:
    lines = [
        "<!-- This file is generated by test/native_api_parity/run_benchmarks_core.py. -->",
        "",
    ...
    for summary in summaries:
        ...
        for test in _report_tests(summary):
            if test.startswith("core_ceiling_") or test.startswith("callin_drawworld_"):
                continue
            metrics = {
                backend: _timed_metric(indexed[backend].get(test, {}), test)
                for backend in REPORT_BACKENDS
            }
            if all(metric is not None for metric in metrics.values()):
                continue
            if not any(indexed[backend].get(test) for backend in REPORT_BACKENDS):
                continue
            values = []
            for backend in REPORT_BACKENDS:
                row = indexed[backend].get(test, {})
                ...
                values.append(
                    base.format_metric(metric)
                    if metric is not None
                    else str(row.get("status", "missing"))
                )
            lines.append(f"| `{profile}` | `{test}` | " + " | ".join(values) + " |")
    lines.append("")
    return "\n".join(lines)


def validate_sample_counts(summary: dict) -> None:
    """Reject a run whose reported sample cardinalities are not comparable."""
    expected = int(summary.get("repeats", 0))
    if expected <= 0:
        raise RuntimeError(f"{summary['profile']} has no positive repeat count")

    by_test: dict[str, set[int]] = {}
    for rows in summary["rows"].values():
        for row in rows:
            samples = next(
                (row[key] for key in ("samplesNs", "samplesMs") if key in row),
                None,
            )
            if samples is None:
                continue
            ...
            if count != expected:
                raise RuntimeError(
                    f"{summary['profile']}/{row.get('test')} reports {count} samples; "
                    f"expected {expected}"
                )
            p99_key = "p99Ns" if "samplesNs" in row else "p99Ms"
            spread_key = "spreadNs" if "samplesNs" in row else "spreadMs"
            if p99_key not in row or spread_key not in row:
                raise RuntimeError(
                    f"{summary['profile']}/{row.get('test')} has samples without "
                    f"{p99_key}/{spread_key}"
                )
            by_test.setdefault(str(row["test"]), set()).add(count)
    mismatched = {
        test: sorted(counts)
        for test, counts in by_test.items()
        if len(counts) != 1
    }
    if mismatched:
        raise RuntimeError(f"uneven sample counts: {mismatched}")


def render_report(summaries: list[dict]) -> str:
    return _render_comparison(_resolved_summaries(summaries))
...
def validate_report_shape(report: str) -> None:
    ...
    if not lines[2].startswith("| Profile | Test | Lua | Native | Core | Core vs Lua |"):
...
def write_results_and_freeze(output_path, summaries: list[dict]) -> None:
    ...
    """
    for summary in summaries:
        validate_sample_counts(summary)
    if os.environ.get("RECOIL_BENCHMARK_CHECK_REGRESSION", "").strip() in {"1", "true", "yes", "on"}:
        failures = [
            failure
            for summary in summaries
            for failure in frozen_benchmarks.regression_failures(summary, FROZEN_ROOT)
        ]
        if failures:
            raise RuntimeError("benchmark regression(s): " + json.dumps(failures, sort_keys=True))
    if FREEZE:
        for summary in summaries:
    ...
    resolved = _resolved_summaries(summaries)
    generated_root = ROOT / "rts" / "wasm" / "docs" / "generated"
    generated_root.mkdir(parents=True, exist_ok=True)
    for name, content in (
        ("benchmarking_losses.md", _render_losses(resolved)),
        ("benchmarking_diagnostics.md", _render_diagnostics(resolved)),
        ("benchmarking_unpaired.md", _render_unpaired(resolved)),
    ):
        (generated_root / name).write_text(content, encoding="utf-8")
