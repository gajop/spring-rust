...
def _percentile(samples: list[float], fraction: float) -> float | None:
    ...
    return values[min(int(fraction * (len(values) - 1)), len(values) - 1)]


def _format_sample(value: float, unit: str) -> str:
    return f"{value:.6f} {unit}" if unit == "ms" else f"{value:.3f} {unit}"


def _sample_stats(row: dict) -> tuple[int | None, str, str, str]:
    samples = row.get("samplesNs") if "medianNs" in row else row.get("samplesMs")
    if isinstance(samples, list):
        ...
        samples = []
    count = len(samples) or (int(row["iterations"]) if row.get("iterations") is not None else None)
    if "medianNs" in row:
        unit, scale = "ns", 1.0
        p50 = float(row["medianNs"])
        p99 = float(row.get("p99Ns", _percentile(samples, 0.99) or 0.0))
        spread = float(row.get("spreadNs", (max(samples) - min(samples)) if samples else 0.0))
    elif "medianMs" in row:
        unit, scale = "ms", 1.0
        p50 = float(row["medianMs"])
        p99 = float(row.get("p99Ms", _percentile(samples, 0.99) or 0.0))
        spread = float(row.get("spreadMs", (max(samples) - min(samples)) if samples else 0.0))
    else:
        return count, "—", "—", "—"
    return count, _format_sample(p50, unit), _format_sample(p99, unit), _format_sample(spread, unit)


def _sample_columns(row: dict, test: str) -> list[str]:
    count, p50, p99, spread = _sample_stats(row)
    metric = (
        base.format_memory_metric(row, test)
        if test in base.MEMORY_TESTS
        else base.format_metric(base.numeric_metric(row, test))
    )
    return [metric, "—" if count is None else str(count), p50, p99, spread]
...
def render_report(summaries: list[dict]) -> str:
    ...
    # frozen data. The active report intentionally excludes the retained CM
    # history files from its comparison columns.
    summaries = [
    ...
    for summary in summaries:
        ...
        for test in _report_tests(summary):
            # New paired rows may not exist in the historical native store.
            # Keep the row in the generated table with an explicit missing
            # metric rather than silently dropping it or inventing a value.
            rows = {backend: indexed[backend].get(test, {}) for backend in REPORT_BACKENDS}
            values = [_sample_columns(rows[backend], test) for backend in REPORT_BACKENDS]
            lua_metric = base.numeric_metric(rows["lua"], test)
            core_metric = base.numeric_metric(rows["wasm_core"], test)
            beats = core_metric is not None and lua_metric is not None and core_metric[1] == lua_metric[1] and core_metric[0] < lua_metric[0]
            lines.append(
                f"| `{profile}` | {float(summary['scale']):g} | `{test}` | "
                + " | ".join(values[0]) + " | "
                + " | ".join(values[1]) + " | "
                + " | ".join(values[2]) + " | "
                + f"{base.ratio_for_test(test, rows['lua'], rows['wasm_core'])} | "
                + f"{'true' if beats else 'false' if core_metric is not None and lua_metric is not None else 'unknown'} |"
            )
    ...
    for summary in summaries:
        ...
        for test in _report_tests(summary):
            ...
            lua_metric = base.numeric_metric(lua, test)
            core_metric = base.numeric_metric(core, test)
            if core_metric is not None and lua_metric is not None and core_metric[1] == lua_metric[1] and core_metric[0] < lua_metric[0]:
                continue
            reason = "missing_metric" if core_metric is None or lua_metric is None else "core_not_lower"
            lines.append(
                f"| true | `{profile}` | `{test}` | {base.format_timed_metric(core, test)} | "
                f"{base.format_timed_metric(lua, test)} | `{reason}` |"
            )
...
def validate_report_shape(report: str) -> None:
    ...
    header_index = 2 if len(lines) > 2 and lines[2].startswith("| Profile | Scale | Test |") else None
