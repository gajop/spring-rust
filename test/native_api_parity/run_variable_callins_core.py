...
The current inner timing regions are intentionally reported as diagnostics, not
as a decision ratio: Lua starts after its arguments have been pushed whereas
Core includes scratch serialization. Wire identical outer event boundaries
before publishing Lua/Core ratios for these two rows.
"""
...
def run(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "timingBoundaries": {
            "lua": "guest call after Lua arguments have been pushed",
            "wasm_core": "scratch serialization plus unchecked Core guest call",
        },
        "tests": list(TESTS),
...
def render(summary: dict) -> str:
    ...
    lines = [
        ...
        "",
        "# Variable-callin transport diagnostics",
        "",
    ...
    lines.extend([
        "",
        "**Do not compute a Lua/Core ratio from this table yet.** The timed regions differ: "
        "Lua starts after argument pushing, while Core includes scratch serialization. "
        "Use the reserved outer `callin_string_event` / `callin_command_event` rows for "
        "the decision comparison once that identical event boundary is wired.",
        "",
