...
impl NativeApiParity {
    pub(crate) fn check_system_control_value(&mut self, message: &Value, label: &str) -> Result<(), String> {
        match base_test_name(label) {
            "is_replay" => {
                let native = self.interface.system_control().is_replay()
                    .map_err(|err| format!("is_replay() failed: {err:?}"))?;
                self.same_bool_if_present(label, message, "isReplay", native)
            }
            "get_game_name" => {
                let native = self.interface.system_control().get_game_name()
                    .map_err(|err| format!("get_game_name() failed: {err:?}"))?;
                self.same_string_if_present(label, message, "gameName", native.as_deref().unwrap_or(""))
            }
            "get_menu_name" => {
                let native = self.interface.system_control().get_menu_name()
                    .map_err(|err| format!("get_menu_name() failed: {err:?}"))?;
                self.same_string_if_present(label, message, "menuName", native.as_deref().unwrap_or(""))
            }
            "get_replay_length" => {
                let (_seconds, success) = self.interface.system_control().get_replay_length()
            ...
            "get_video_capturing_mode" => {
                let native = self.interface.system_control().get_video_capturing_mode()
                    .map_err(|err| format!("get_video_capturing_mode() failed: {err:?}"))?;
                self.same_bool_if_present(label, message, "allowRecord", native)
            }
            "get_gather_mode" => {
                let native = self.interface.system_control().get_gather_mode()
            ...
            "get_window_display_mode" => {
                ...
                self.same_string_if_present(label, message, "pixelFormatName", format_name.as_deref().unwrap_or(""))
            }
