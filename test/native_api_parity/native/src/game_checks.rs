...
impl NativeApiParity {
    ...
    pub(crate) fn check_game_seconds(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let native = self
            .interface
            .game()
            .get_game_seconds()
    ...
    pub(crate) fn check_gaia_team_id(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let native = self
            .interface
            .game()
            .get_gaia_team_id()
    ...
    pub(crate) fn check_heading_from_facing(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_facing_from_heading(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_heading_from_vector(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_game_bool(&mut self, message: &Value, label: &str) -> Result<(), String> {
        ...
        let native = match test_name {
            "is_cheating_enabled" => self.interface.game().is_cheating_enabled()
                .map_err(|err| format!("is_cheating_enabled() failed: {err:?}"))?,
            "are_helper_ais_enabled" => self.interface.game().are_helper_ais_enabled()
                .map_err(|err| format!("are_helper_ais_enabled() failed: {err:?}"))?,
            "fixed_allies" => self.interface.game().fixed_allies()
                .map_err(|err| format!("fixed_allies() failed: {err:?}"))?,
            "is_game_over" => self.interface.game().is_game_over()
                .map_err(|err| format!("is_game_over() failed: {err:?}"))?,
            "is_god_mode_enabled" => self.interface.game().is_god_mode_enabled()
                .map_err(|err| format!("is_god_mode_enabled() failed: {err:?}"))?,
            "is_dev_lua_enabled" => self.interface.game().is_dev_lua_enabled()
                .map_err(|err| format!("is_dev_lua_enabled() failed: {err:?}"))?,
            "is_edit_defs_enabled" => self.interface.game().is_edit_defs_enabled()
                .map_err(|err| format!("is_edit_defs_enabled() failed: {err:?}"))?,
            "is_no_cost_enabled" => self.interface.game().is_no_cost_enabled()
                .map_err(|err| format!("is_no_cost_enabled() failed: {err:?}"))?,
            _ => return Err(format!("unsupported game bool check `{label}`")),
        };
        let field = if test_name == "is_game_over" { "isGameOver" } else { "enabled" };
    ...
    pub(crate) fn check_game_option(&mut self, message: &Value, label: &str) -> Result<(), String> {
        ...
        let (native, exists) = match test_name {
            "get_mod_option" => self.interface.game().get_mod_option(key)
                .map_err(|err| format!("get_mod_option({key}) failed: {err:?}"))?,
            "get_map_option" => self.interface.game().get_map_option(key)
    ...
    pub(crate) fn check_game_options(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_mod_options" => self.interface.game().get_mod_options()
                .map_err(|err| format!("get_mod_options() failed: {err:?}"))?,
            "get_map_options" => self.interface.game().get_map_options()
    ...
    pub(crate) fn check_game_rules_info(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let rules = self
            .interface
            .game()
            .get_game_rules_info()
        ...
        self.same_i32_if_present(label, message, "constructionDecayTime", rules.constructionDecayTime)?;
        self.same_if_present(label, message, "constructionDecaySpeed", rules.constructionDecaySpeed)?;
        ...
        self.same_if_present(label, message, "reclaimUnitEnergyCostFactor", rules.reclaimUnitEnergyCostFactor)?;
        self.same_if_present(label, message, "reclaimUnitEnergyCostFactor", resources.reclaimUnitCostFactor.energy)?;
        self.same_if_present(label, message, "reclaimUnitEfficiency", rules.reclaimUnitEfficiency)?;
        self.same_if_present(label, message, "reclaimUnitEfficiency", resources.reclaimUnitEfficiency.metal)?;
        self.same_if_present(label, message, "reclaimFeatureEnergyCostFactor", rules.reclaimFeatureEnergyCostFactor)?;
        self.same_if_present(label, message, "reclaimFeatureEnergyCostFactor", resources.reclaimFeatureCostFactor.energy)?;
        self.same_bool_if_present(label, message, "reclaimUnitDrainHealth", rules.reclaimUnitDrainHealth)?;
        self.same_bool_if_present(label, message, "reclaimAllowEnemies", rules.reclaimAllowEnemies)?;
        self.same_bool_if_present(label, message, "reclaimAllowAllies", rules.reclaimAllowAllies)?;
        self.same_if_present(label, message, "repairEnergyCostFactor", rules.repairEnergyCostFactor)?;
        self.same_if_present(label, message, "repairEnergyCostFactor", resources.repairCostFactor.energy)?;
        self.same_if_present(label, message, "resurrectEnergyCostFactor", rules.resurrectEnergyCostFactor)?;
        self.same_if_present(label, message, "resurrectEnergyCostFactor", resources.resurrectCostFactor.energy)?;
        self.same_if_present(label, message, "captureEnergyCostFactor", rules.captureEnergyCostFactor)?;
        self.same_if_present(label, message, "captureEnergyCostFactor", resources.captureCostFactor.energy)?;
        ...
        self.same_i32_if_present(label, message, "requireSonarUnderWater", rules.requireSonarUnderWater)?;
        self.same_bool_if_present(label, message, "paralyzeOnMaxHealth", rules.paralyzeOnMaxHealth)?;
        self.same_if_present(label, message, "paralyzeDeclineRate", rules.paralyzeDeclineRate)?;
        self.same_bool_if_present(label, message, "allowEnginePlayerlist", rules.allowEnginePlayerlist)?;
        self.same_bool_if_present(label, message, "nativeExcessSharing", rules.nativeExcessSharing)
    }
    ...
    pub(crate) fn check_side_data_by_index(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let side_index = i32_field(message, "sideIndex")?;
        let native_index = side_index.checked_sub(1).ok_or_else(|| format!("{label}.sideIndex must be one-based"))? as u32;
        let native = self
            .interface
            .game()
    ...
    pub(crate) fn check_side_data_count(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let native = self
            .interface
            .game()
            .get_side_data_count()
    ...
    fn same_side_data(&mut self, label: &str, message: &Value, native: sys::SideData) -> Result<(), String> {
        let case_name = unsafe { native.caseName.as_ref().map(|ptr| CStr::from_ptr(ptr).to_string_lossy().into_owned()).unwrap_or_default() };
        let start_unit = unsafe { native.startUnit.as_ref().map(|ptr| CStr::from_ptr(ptr).to_string_lossy().into_owned()).unwrap_or_default() };
        let normalized_side_name = unsafe { native.sideName.as_ref().map(|ptr| CStr::from_ptr(ptr).to_string_lossy().into_owned()).unwrap_or_default() };
    ...
    pub(crate) fn check_game_global_los(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_game_vector_from_heading(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let heading = i32_field(message, "heading")?;
        let native = self
            .interface
            .game()
    ...
    pub(crate) fn check_game_ally_team_start_box(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let ally_team_id = i32_field(message, "allyTeamID")?;
    ...
    pub(crate) fn check_game_team_start_position(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_map_start_positions(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let native = self
            .interface
            .game()
