...
impl NativeApiParity {
    pub(crate) fn check_unit_storage_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_stockpile_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let unit_id = i32_field(message, "unitID")?;
        ...
        self.same_i32_if_present(label, message, "stockpileQueueSize", stockpile.stockpileQueueSize as i32)?;
        self.same_if_present(label, message, "buildPercent", stockpile.buildPercent)
    }

    pub(crate) fn check_unit_is_transporting_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let unit_id = i32_field(message, "unitID")?;
    ...
    pub(crate) fn check_unit_pos_error_params_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let unit_id = i32_field(message, "unitID")?;
        ...
        let native = self
            ...
            .map_err(|err| format!("get_unit_pos_error_params({unit_id}, {ally_team_id}) failed: {err:?}"))?;
        ...
        self.same_i32_if_present(label, message, "nextPosErrorUpdate", native.nextPosErrorUpdate)?;
        self.same_bool_if_present(label, message, "posErrorBit", native.posErrorBit)
    }

    pub(crate) fn check_unit_flanking_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let unit_id = i32_field(message, "unitID")?;
        let native = self
            .interface
            .units_info()
    ...
    pub(crate) fn check_unit_last_hit_piece_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let unit_id = i32_field(message, "unitID")?;
        let native = self
            .interface
            .units_info()
        ...
        self.compare_last_hit_piece(label, message, native.name, native.pieceNum, native.frame, native.wasHit)
    }

    pub(crate) fn check_feature_last_hit_piece_fixed(&mut self, message: &Value, label: &str) -> Result<(), String> {
        let feature_id = i32_field(message, "featureID")?;
        let native = self
            .interface
            .features()
            .get_feature_last_attacked_piece(feature_id)
            .map_err(|err| format!("get_feature_last_attacked_piece({feature_id}) failed: {err:?}"))?;
        self.compare_last_hit_piece(label, message, native.name, native.pieceNum, native.frame, native.wasHit)
    }
