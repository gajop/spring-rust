...
impl NativeApiParity {
    pub(crate) fn check_config_value(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        match test_name {
            "get_config_int" | "get_config_int_nil_default" | "config_int" => {
                ...
                let (native, exists) = self
                    ...
                    .map_err(|err| {
                        format!(
                            "get_config_int({key}, {default_value}, {has_default}) failed: {err:?}"
                        )
                    })?;
    ...
    pub(crate) fn set_config_value(&mut self, message: &Value) -> Result<(), String> {
        ...
        match base_test_name(str_field(message, "name")?) {
            "config_int" => {
                ...
                self.interface
                    .config()
                    .set_config_int(key, value, use_overlay)
            ...
            "config_float" => {
                ...
                self.interface
                    .config()
                    .set_config_float(key, value, use_overlay)
            ...
            "config_string" => {
                ...
                self.interface
                    .config()
                    .set_config_string(key, value, use_overlay)
