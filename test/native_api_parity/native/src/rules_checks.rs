...
impl NativeApiParity {
    pub(crate) fn check_rules_param(&mut self, message: &Value, label: &str) -> Result<(), String> {
        ...
        let (value, _, exists, names) = match scope {
            ...
            "team" => {
                ...
                let (value, los, exists) = rules
                    ...
                    .map_err(|err| {
                        format!("get_team_rules_param({team_id}, {param_name}) failed: {err:?}")
                    })?;
                let names = rules
            ...
            "player" => {
                ...
                let (value, los, exists) = rules
                    ...
                    .map_err(|err| {
                        format!("get_player_rules_param({player_id}, {param_name}) failed: {err:?}")
                    })?;
                let names = rules.get_player_rules_params(player_id).map_err(|err| {
                    format!("get_player_rules_params({player_id}) failed: {err:?}")
                })?;
                (value, los, exists, names)
            }
            "unit" => {
                ...
                let (value, los, exists) = rules
                    ...
                    .map_err(|err| {
                        format!("get_unit_rules_param({unit_id}, {param_name}) failed: {err:?}")
                    })?;
                let names = rules
            ...
            "feature" => {
                ...
                let (value, los, exists) = rules
                    ...
                    .map_err(|err| {
                        format!(
                            "get_feature_rules_param({feature_id}, {param_name}) failed: {err:?}"
                        )
                    })?;
                let names = rules.get_feature_rules_params(feature_id).map_err(|err| {
                    format!("get_feature_rules_params({feature_id}) failed: {err:?}")
                })?;
                (value, los, exists, names)
            }
        ...
        if !exists {
            return Err(format!(
                "{label}.{param_name}: native rules param does not exist"
            ));
        }
        ...
        self.same_bool_if_present(
            ...
            message,
            "listed",
            names.iter().any(|name| name == param_name),
        )
    }
        ...
        match str_field(message, "scope")? {
            ...
            "team" => {
                ...
                rules
                    ...
                    .map_err(|err| {
                        format!("set_team_rules_param({team_id}, {param_name}) failed: {err:?}")
                    })?
            }
            "player" => {
                let player_id = i32_field(message, "playerID")?;
                rules
                    ...
                    .map_err(|err| {
                        format!("set_player_rules_param({player_id}, {param_name}) failed: {err:?}")
                    })?
            }
            "unit" => {
                let unit_id = i32_field(message, "unitID")?;
                rules
                    ...
                    .map_err(|err| {
                        format!("set_unit_rules_param({unit_id}, {param_name}) failed: {err:?}")
                    })?
            }
            "feature" => {
                let feature_id = i32_field(message, "featureID")?;
                rules
                    ...
                    .map_err(|err| {
                        format!(
                            "set_feature_rules_param({feature_id}, {param_name}) failed: {err:?}"
                        )
                    })?
            }
            other => return Err(format!("unsupported rules param scope `{other}`")),
