...
impl NativeApiParity {
    pub(crate) fn check_unit_defs_list_count(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_defs_bool(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_defs_string(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_defs_f32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let unit_def_id = i32_field(message, "unitDefID")?;
        let native = match test_name {
            "get_unit_def_health" => self
                .interface
                .unit_defs()
                .get_unit_def_health(unit_def_id)
                .map_err(|err| format!("get_unit_def_health({unit_def_id}) failed: {err:?}"))?,
            "get_unit_def_speed" => self
                .interface
                .unit_defs()
                .get_unit_def_speed(unit_def_id)
            ...
            "get_unit_def_metal_cost" | "get_unit_def_energy_cost" | "get_unit_def_build_time" => {
                let costs = self
                    .interface
                    .unit_defs()
                    .get_unit_def_costs(unit_def_id)
    ...
    pub(crate) fn check_unit_defs_keys(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            .interface
            .unit_defs()
            .get_unit_def_custom_param_keys(unit_def_id)
            .map_err(|err| {
                format!("get_unit_def_custom_param_keys({unit_def_id}) failed: {err:?}")
            })?;
        self.same_string_set_if_present(label, message, "keys", &native)
    }

    pub(crate) fn check_unit_def_by_id(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_feature_defs_list_count(
        ...
        match test_name {
            ...
            _ => Err(format!(
                "unsupported feature defs list/count check `{label}`"
            )),
        }
    }

    pub(crate) fn check_feature_defs_bool(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            .interface
            .feature_defs()
            .valid_feature_def_id(feature_def_id)
            .map_err(|err| format!("valid_feature_def_id({feature_def_id}) failed: {err:?}"))?;
        self.same_bool_if_present(label, message, "valid", native)
    }

    pub(crate) fn check_feature_defs_i32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_feature_defs_string(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_feature_def_name" => self
                .interface
                .feature_defs()
                .get_feature_def_name(feature_def_id)
                .map_err(|err| format!("get_feature_def_name({feature_def_id}) failed: {err:?}"))?,
            "get_feature_def_custom_param" => {
                let key = str_field(message, "key")?;
                self.interface
                    .feature_defs()
                    .get_feature_def_custom_param(feature_def_id, key)
                    .map_err(|err| {
                        format!(
                            "get_feature_def_custom_param({feature_def_id}, {key}) failed: {err:?}"
                        )
                    })?
            }
    ...
    pub(crate) fn check_feature_defs_f32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_feature_def_metal" => self
                .interface
                .feature_defs()
                .get_feature_def_metal(feature_def_id)
                .map_err(|err| {
                    format!("get_feature_def_metal({feature_def_id}) failed: {err:?}")
                })?,
            "get_feature_def_energy" => self
                .interface
                .feature_defs()
                .get_feature_def_energy(feature_def_id)
                .map_err(|err| {
                    format!("get_feature_def_energy({feature_def_id}) failed: {err:?}")
                })?,
    ...
    pub(crate) fn check_feature_defs_keys(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            .interface
            .feature_defs()
            .get_feature_def_custom_param_keys(feature_def_id)
            .map_err(|err| {
                format!("get_feature_def_custom_param_keys({feature_def_id}) failed: {err:?}")
            })?;
        self.same_string_set_if_present(label, message, "keys", &native)
    }

    pub(crate) fn check_feature_def_by_id(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let (info, exists) = self
            .interface
            .feature_defs()
            .get_feature_def_by_id(feature_def_id)
    ...
    pub(crate) fn check_weapon_defs_list_count(
        ...
        match test_name {
            ...
            _ => Err(format!(
                "unsupported weapon defs list/count check `{label}`"
            )),
        }
    }

    pub(crate) fn check_weapon_defs_bool(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            .interface
            .weapon_defs()
            .valid_weapon_def_id(weapon_def_id)
            .map_err(|err| format!("valid_weapon_def_id({weapon_def_id}) failed: {err:?}"))?;
        self.same_bool_if_present(label, message, "valid", native)
    }

    pub(crate) fn check_weapon_defs_i32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_weapon_defs_string(
        ...
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_weapon_def_name" => self
                .interface
                .weapon_defs()
                .get_weapon_def_name(weapon_def_id)
                .map_err(|err| format!("get_weapon_def_name({weapon_def_id}) failed: {err:?}"))?,
            "get_weapon_def_custom_param" => {
                let key = str_field(message, "key")?;
                self.interface
                    .weapon_defs()
                    .get_weapon_def_custom_param(weapon_def_id, key)
                    .map_err(|err| {
                        format!(
                            "get_weapon_def_custom_param({weapon_def_id}, {key}) failed: {err:?}"
                        )
                    })?
            }
    ...
    pub(crate) fn check_weapon_defs_f32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_weapon_def_range" => self
                .interface
                .weapon_defs()
                .get_weapon_def_range(weapon_def_id)
                .map_err(|err| format!("get_weapon_def_range({weapon_def_id}) failed: {err:?}"))?,
            "get_weapon_def_damage" => self
                .interface
                .weapon_defs()
                .get_weapon_def_damage(weapon_def_id)
    ...
    pub(crate) fn check_weapon_defs_keys(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            .interface
            .weapon_defs()
            .get_weapon_def_custom_param_keys(weapon_def_id)
            .map_err(|err| {
                format!("get_weapon_def_custom_param_keys({weapon_def_id}) failed: {err:?}")
            })?;
        self.same_string_set_if_present(label, message, "keys", &native)
    }

    pub(crate) fn check_weapon_def_by_id(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let (info, exists) = self
            .interface
            .weapon_defs()
            .get_weapon_def_by_id(weapon_def_id)
