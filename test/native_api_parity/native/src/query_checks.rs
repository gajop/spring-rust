...
impl NativeApiParity {
    ...
    pub(crate) fn check_units_query_count(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = match test_name {
            "get_team_unit_count" => self
                .interface
                .units_query()
                .get_team_unit_count(team_id)
            ...
            "get_team_unit_def_count" => {
                ...
                self.interface
                    .units_query()
                    .get_team_unit_def_count(team_id, unit_def_id)
                    .map_err(|err| {
                        format!("get_team_unit_def_count({team_id}, {unit_def_id}) failed: {err:?}")
                    })?
            }
    ...
    pub(crate) fn check_units_query_list(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = match test_name {
            ...
            "get_team_units_by_defs" => {
                ...
                self.interface
                    .units_query()
                    .get_team_units_by_defs(team_id, &[unit_def_id])
                    .map_err(|err| {
                        format!(
                            "get_team_units_by_defs({team_id}, [{unit_def_id}]) failed: {err:?}"
                        )
                    })?
            }
            "get_render_units" => {
                ...
                self.interface
                    .units_query()
                    .get_render_units(draw_mask, send_mask)
                    .map_err(|err| {
                        format!("get_render_units({draw_mask}, {send_mask}) failed: {err:?}")
                    })?
            }
            "get_render_units_draw_flag_changed" => {
                ...
                self.interface
                    .units_query()
                    .get_render_units_draw_flag_changed(send_mask)
                    .map_err(|err| {
                        format!("get_render_units_draw_flag_changed({send_mask}) failed: {err:?}")
                    })?
            }
    ...
    pub(crate) fn check_units_query_counts(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_team_units_sorted(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_units_query_set(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_separation(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_feature_separation(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_unit_centroid(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let test_name = base_test_name(label);
        let native = match test_name {
            "get_unit_array_centroid" => self
                .interface
                .units_query()
                .get_unit_array_centroid(&[unit_id])
                .map_err(|err| format!("get_unit_array_centroid([{unit_id}]) failed: {err:?}"))?,
            "get_unit_map_centroid" => self
                .interface
                .units_query()
                .get_unit_map_centroid(&[unit_id])
    ...
    pub(crate) fn check_units_query_i32(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let (field, native) = match test_name {
            "get_unit_nearest_ally" => {
                ...
                (
                    ...
                    self.interface
                        .units_query()
                        .get_unit_nearest_ally(unit_id, range)
                        .map_err(|err| {
                            format!("get_unit_nearest_ally({unit_id}, {range}) failed: {err:?}")
                        })?,
                )
            }
    ...
    pub(crate) fn check_features_bool(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_features_list(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_feature_identity(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_teams_bool(&mut self, message: &Value, label: &str) -> Result<(), String> {
        ...
        let native = match test_name {
            "are_teams_allied" | "set_ally" => {
                ...
                self.interface
                    .teams()
                    .are_teams_allied(team_id1, team_id2)
                    .map_err(|err| {
                        format!("are_teams_allied({team_id1}, {team_id2}) failed: {err:?}")
                    })?
            }
            "are_players_allied" => {
                ...
                self.interface
                    .teams()
                    .are_players_allied(player_id1, player_id2)
                    .map_err(|err| {
                        format!("are_players_allied({player_id1}, {player_id2}) failed: {err:?}")
                    })?
            }
        ...
        let (field, native) = match test_name {
            ...
            "get_player_list" => {
                ...
                (
                    ...
                    self.interface
                        .teams()
                        .get_player_list(team_id, active)
                        .map_err(|err| {
                            format!("get_player_list({team_id}, {active}) failed: {err:?}")
                        })?,
                )
            }
            "get_player_list_in_team" => {
                ...
                (
                    ...
                    self.interface
                        .teams()
                        .get_player_list_in_team(team_id)
                        .map_err(|err| {
                            format!("get_player_list_in_team({team_id}) failed: {err:?}")
                        })?,
                )
            }
            "get_player_list_in_ally_team" => {
                ...
                (
                    ...
                    self.interface
                        .teams()
                        .get_player_list_in_ally_team(ally_team_id)
                        .map_err(|err| {
                            format!("get_player_list_in_ally_team({ally_team_id}) failed: {err:?}")
                        })?,
                )
            }
    ...
    pub(crate) fn check_ai_info_fixed(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_ally_team_info(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_team_resources(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_team_resource_stats(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        ...
        let native = self
            ...
            .map_err(|err| {
                format!("get_team_resource_stats({team_id}, {resource}) failed: {err:?}")
            })?;
        if resource.starts_with('m') {
    ...
    pub(crate) fn check_team_unit_stats(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn check_team_stats_history_count(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
    ...
    pub(crate) fn set_team_share_level(&mut self, message: &Value) -> Result<(), String> {
        ...
        self.interface
            ...
            .map_err(|err| {
                format!("set_team_share_level({team_id}, {resource}, {share}) failed: {err:?}")
            })?;
        Ok(())
    }
    ...
    pub(crate) fn add_team_resource(&mut self, message: &Value) -> Result<(), String> {
        ...
        team.set_team_resource(team_id, resource, baseline)
            .map_err(|err| {
                format!("set_team_resource({team_id}, {resource}, {baseline}) failed: {err:?}")
            })?;
        team.add_team_resource(team_id, resource, amount)
            .map_err(|err| {
                format!("add_team_resource({team_id}, {resource}, {amount}) failed: {err:?}")
            })?;
        Ok(())
    }
    pub(crate) fn use_team_resource(&mut self, message: &Value) -> Result<(), String> {
        ...
        team.use_team_resource(team_id, resource, amount)
            .map_err(|err| {
                format!("use_team_resource({team_id}, {resource}, {amount}) failed: {err:?}")
            })?;
        Ok(())
    }
}
