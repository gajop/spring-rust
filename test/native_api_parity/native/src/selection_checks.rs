...
impl NativeApiParity {
    pub(crate) fn check_selection_value(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        match base_test_name(label) {
            ...
            "get_group_units_count" => {
                ...
                let native = self
                    .interface
                    .selection()
                    .get_group_units_count(group_id)
            ...
            "get_group_units_counts" => {
                ...
                let native = self
                    .interface
                    .selection()
                    .get_group_units_counts(group_id)
            ...
            "get_group_units_sorted_fixed" => {
                ...
                let native = self
                    .interface
                    .selection()
                    .get_group_units_sorted(group_id)
        ...
        match base_test_name(str_field(message, "name")?) {
            ...
            "select_unit_array" => {
                ...
                let success = self
                    .interface
                    .selection()
                    .select_unit_array(&[unit_id], append)
                    .map_err(|err| {
                        format!("select_unit_array([{unit_id}], {append}) failed: {err:?}")
                    })?;
                if success {
                    Ok(())
                } else {
                    Err(format!(
                        "select_unit_array([{unit_id}], {append}) returned false"
                    ))
                }
            }
            "deselect_unit_array" => {
            ...
            "select_unit_map" => {
                ...
                let success = self
                    .interface
                    .unsynced_ctrl()
                    .select_unit_map(&[unit_id], append)
                    .map_err(|err| {
                        format!("select_unit_map([{unit_id}], {append}) failed: {err:?}")
                    })?;
                if success {
                    Ok(())
                } else {
                    Err(format!(
                        "select_unit_map([{unit_id}], {append}) returned false"
                    ))
                }
            }
            "deselect_unit_map" => {
            ...
            | "get_group_units_sorted_fixed" => {
                ...
                let success = self
                    .interface
                    .selection()
                    .set_unit_group(unit_id, group_id)
                    .map_err(|err| {
                        format!("set_unit_group({unit_id}, {group_id}) failed: {err:?}")
                    })?;
                if success {
                    Ok(())
                } else {
                    Err(format!(
                        "set_unit_group({unit_id}, {group_id}) returned false"
                    ))
                }
            }
