...
impl NativeApiParity {
    pub(crate) fn check_player_value(
        &mut self,
        message: &Value,
        label: &str,
    ) -> Result<(), String> {
        match base_test_name(label) {
            ...
            "get_player_traffic_missing_packet" => {
                ...
                let native = self
                    .interface
                    .player()
                    .get_player_traffic(player_id, packet_id)
                    .map_err(|err| {
                        format!("get_player_traffic({player_id}, {packet_id}) failed: {err:?}")
                    })?;
                let traffic = native
                    .first()
                    .map(|traffic| {
                        if traffic.bytesSent == u32::MAX {
                            -1
                        } else {
                            traffic.bytesSent as i32
                        }
                    })
                    .unwrap_or(-1);
